...
def _is_lr_scheduler_class(
    ...
    # ReduceLROnPlateau is NOT a subclass of LRScheduler but it is a
    # PyTorch LR scheduler by definition.
    return is_subclass(lr_scheduler_class, LRScheduler) or (
        lr_scheduler_class is ReduceLROnPlateau
    )
...
def get_lr_scheduler(
    ...
    try:
        ...
        try:
            ...
            _lr_scheduler_class: type = get_class_from_module(  # type: ignore
                class_name=lr_scheduler_name,
            ...
            _error_msg = (
                "Cannot find an learning rate scheduler with a name "
            ...
            _error_msg = (
                "Cannot find an learning rate scheduler with a name "
                f"equal (or similar) to '{lr_scheduler_name}' in "
                "module `torch.optim.lr_scheduler` or "
                "`flash.core.optimizers`."
            )
            raise ValueError(_error_msg)
    if not _is_lr_scheduler_class(_lr_scheduler_class):
        _warning_msg = (
