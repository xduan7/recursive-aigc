...
def __get_available_gpus() -> List[Device]:
    ...
    Returns: A list of GPU indices or computation that PyTorch has access to.
    ...
    if "CUDA_VISIBLE_DEVICES" in os.environ:
        _available_gpus = __reindex_available_gpus_by_visible_devices(
            _available_gpus
        )
...
def __reindex_available_gpus_by_visible_devices(
    ...
    However, PyTorch can only have access to visible computation, and the
    ...
    _cuda_visible_devices = [
        int(__d) for __d in os.environ["CUDA_VISIBLE_DEVICES"].split(",")
    ]
    ...
    for __available_gpu in available_gpus:
        try:
            ...
            _warning_msg = (
                ...
                "available by GPUtil and `CUDA_VISIBLE_DEVICES`, "
                "is not accessible by PyTorch."
            )
...
def get_all_devices() -> List[torch.device]:
    """Get all computation devices (CPU or GPUs) available.
    ...
    Returns: A list of computation computation (``torch.device``),
